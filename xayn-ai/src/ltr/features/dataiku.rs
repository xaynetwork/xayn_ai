...
impl<'de> serde::Deserialize<'de> for Rank {
    ...
    {
        let rank = <usize>::deserialize(deserializer)?;
        //panics, but that is ok as it is #[cfg(test)]
        Ok(Rank::from_usize(rank))
    }
}
...
impl<'de> serde::Deserialize<'de> for DayOfWeek {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        let day_idx = <usize>::deserialize(deserializer)?;
        let day = [Mon, Tue, Wed, Thu, Fri, Sat, Sun][day_idx % 7];
        Ok(day)
    }
}
...
pub struct CurrentSearchResult {
    ...
    #[cfg_attr(test, serde(deserialize_with = "deser_i32_vec_from_string"))]
...
#[cfg(test)]
fn deser_i32_vec_from_string<'de, D>(deserializer: D) -> Result<Vec<i32>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    use serde::Deserialize;

    let unsplit = String::deserialize(deserializer)?;
    let vec = unsplit
        .split(',')
...
mod tests {
    ...
    */
    ...
    fn read_test_history(path: impl AsRef<Path>) -> Vec<SearchResult> {
        let mut reader = csv::Reader::from_path(path).unwrap();
        reader
            .deserialize()
            .map(|record: Result<SearchResult, _>| record.unwrap())
            .collect()
    }

    fn read_test_query(path: impl AsRef<Path>) -> Vec<CurrentSearchResult> {
        let mut reader = csv::Reader::from_path(path).unwrap();
        reader
            .deserialize()
            .map(|record: Result<CurrentSearchResult, _>| record.unwrap())
            .collect()
    }
