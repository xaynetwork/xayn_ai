...
- The `wasmParallel` `Feature` has been the replaced by the corresponding `WasmFeature`s. A caller only needs to pass the set of available features to the `getAssets` function and the library automatically returns the most suitable WASM assets for the given features.
