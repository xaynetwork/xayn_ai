...
- Add multi-threading support: Parallelism is always enabled for mobile targets and can optionally be enabled for web targets. A `FeatureHint` is now required to pick the assets for the chosen target during the setup (see `getInputData()`), available choices are `mobileParallel`, `webSequential` and `webParallel`. The asset generation and publishing has been updated accordingly.
